...
课程演示的网络实验经 ADR-012 单独授权：在 V0.2 提前实现一个仅绑定本机的 MCP Streamable HTTP ToolLab 只读工具服务及客户端 Adapter，用固定任务对比本地与远程工具调用。任务、模型、预算和评测器保持一致；记录端到端时延与失败，结果标记传输方式。该例外不提前开放任意 MCP Server、真实写工具、OAuth、人工审批或 V0.5 的完整 MCP 产品能力，也不改变 V0.1 验收口径。

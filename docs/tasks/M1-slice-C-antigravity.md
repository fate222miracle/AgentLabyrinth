# M1 切片 C：真实 BFCL 子集接入

SSOT：`docs/product/requirements.md` V0.5（M1 例外增补）；ADR-003 Accepted。本卡授权 Antigravity 实现**一个真实、固定、非 Live 的 BFCL 改编子集**，从同一 Web 入口运行并查看结果。Codex 已复核切片 B：修复前端 `artifact.trace.events` 与实际 `artifact.events` 的不匹配，并实测浏览器 Fake 成功/失败、真实 Gemini 成功及刷新回读。切片 B 的网页不等于已接入公开数据集。
...
4. 交付变更文件清单、源码阅读顺序、调用链、状态变化、失败路径、实际实验结果与 README 复现命令；更新 `docs/handoffs/current-state.md`。Codex 复核来源/许可、评分口径、实际请求与测试后签收。
